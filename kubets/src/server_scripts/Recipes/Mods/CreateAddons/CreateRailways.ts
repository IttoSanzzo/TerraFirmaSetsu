...
export function setRecipesCreateRailways(event: $RecipesEventJS) {
	event.replaceInput(
		{ mod: "railways" },
		"minecraft:campfire",
		"#tfc:firepit_fuel"
				...
				sequence: [
					{
						...
						ingredients: [
							{ item: `railways:track_incomplete_tfc_${wood}` },
							[{ tag: "forge:rods/wrought_iron" }, { tag: "forge:rods/zinc" }],
						],
						results: [{ item: `railways:track_incomplete_tfc_${wood}` }],
					},
					{
						type: "create:deploying",
						ingredients: [
					...
					},
					{
						...
						ingredients: [{ item: `railways:track_incomplete_tfc_${wood}` }],
						results: [{ item: `railways:track_incomplete_tfc_${wood}` }],
					},
				],
				...
				sequence: [
					{
						type: "create:cutting",
						ingredients: [
							{ item: `railways:track_incomplete_tfc_${wood}_narrow` },
						],
						results: [{ item: `railways:track_incomplete_tfc_${wood}_narrow` }],
					},
					{
						type: "create:deploying",
						ingredients: [
	...
	ItemCol.tfcWoodTypes.forEach((wood) => {
		...
		newTrackNormal(wood);
		event.remove({
			id: `railways:sequenced_assembly/track_tfc_${wood}_narrow`,
		});
		newTrackNarrow(wood);
	});
}
