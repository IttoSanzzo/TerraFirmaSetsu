...
function setRecipesHexerei(event) {
	...
	event.replaceInput({ mod: "hexerei" }, "minecraft:glow_berries", "#hexerei:tallow_meltable");
	event.replaceInput({ mod: "hexerei" }, "minecraft:honeycomb", "firmalife:beeswax");
	...
	event.shapeless("hexerei:sage_seed", [
		"9x #forge:seeds"
	...
	const reWood = (name, woodList) => {
		...
		woodList.forEach((wood) => {
			addDamageShapeless(event, `4x hexerei:${name}_planks`, [`hexerei:${wood}`, "#tfc:metal/saws"]);
		});
	}
	reWood("willow", [
		"willow_wood",
	...
	const addRawCauldron = (output, ingredient) => {
		...
		}).id(`setsu:hexerei/raw_${output.replace(":", "_").replace("/", "_")}_transformation`);
	};
	...
	addRawCauldron("immersiveengineering:raw_silver", "tfc:ore/normal_native_silver");
}
