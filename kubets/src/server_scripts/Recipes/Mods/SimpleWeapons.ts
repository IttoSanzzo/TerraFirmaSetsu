...
export function setRecipesSimpleWeapons(event: $RecipesEventJS) {
	...
	const smithAllWeapons = (
		...
		if (name == "diamond" || name == "pendorite" || name == "netherite")
			spear = "halberd";
		weaponSmith(
			`simple_weapons:${name}_scythe`,
			ingredientHeavy,
			["draw_last", "upset_second_last", "bend_third_last"],
			tier
		);
		weaponSmith(
			`simple_weapons:${name}_dagger`,
			ingredientLight,
			["draw_last", "shrink_second_last", "shrink_third_last"],
			tier
		);
		weaponSmith(
			`simple_weapons:${name}_greatsword`,
			ingredientHeavy,
			["shrink_last", "draw_second_last", "draw_third_last"],
			tier
		);
		weaponSmith(
			`simple_weapons:${name}_${spear}`,
			ingredientLight,
			["draw_last", "hit_second_last", "hit_third_last"],
			tier
		);
		weaponSmith(
			`simple_weapons:${name}_fist`,
			ingredientLight,
			["hit_last", "hit_second_last", "shrink_third_last"],
			tier
		);
		weaponSmith(
			`simple_weapons:${name}_scimitar`,
			ingredientLight,
			["hit_last", "bend_second_last", "draw_third_last"],
			tier
		);
		weaponSmith(
			`simple_weapons:${name}_sickle`,
			ingredientHeavy,
			["shrink_last", "hit_second_last", "punch_third_last"],
			tier
		);
		weaponSmith(
			`simple_weapons:${name}_katana`,
			ingredientHeavy,
			["shrink_last", "bend_not_last", "bend_not_last"],
			tier
		);
	};
	...
	simpleArmTypes.forEach((type) => {
		...
		event
			.custom({
				...
				items: [`simple_weapons:netherite_${source}`, "mna:greater_mote_fire"],
				patterns: ["mna:circle", "mna:star", "mna:diamond"],
			})
		...
		event
			.custom({
				...
				items: [`simple_weapons:pendorite_${source}`, "mna:greater_mote_ender"],
				patterns: ["mna:circle", "mna:hourglass", "mna:diamond"],
			})
