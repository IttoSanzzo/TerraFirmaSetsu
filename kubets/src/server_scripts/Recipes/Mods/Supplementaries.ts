...
export function setRecipesSupplementaries(event: $RecipesEventJS) {
	...
	event
		.shapeless("supplementaries:flax_block", ["8x #forge:straw"])
		.id("setsu:supplementaries/flax_block");
	event
		.shapeless("8x tfc:straw", ["supplementaries:flax_block"])
		.id("setsu:supplementaries/flax_block_uncrafting");

	event.remove({ id: "supplementaries:candle_holders/candle_holder" });
	event
		.shaped("supplementaries:candle_holder", ["C ", "I "], {
			C: "tfc:candle",
	...
	ItemCol.allDyes.forEach((color) => {
		...
		event
			.shaped(`supplementaries:candle_holder_${color}`, ["C ", "I "], {
				C: `tfc:candle/${color}`,
				I: "tfc:metal/ingot/wrought_iron",
			})
	...
	event
		.shaped("8x supplementaries:daub", ["DDD", "DXD", "DDD"], {
			X: "tfc:mortar",
			D: "tfc:daub",
		})
	...
	event
		.shaped("supplementaries:safe", [" M ", "MBM", " M "], {
	...
	event
		.shaped("supplementaries:globe", [" BN", "NNN", " P "], {
	...
	event
		.shaped("supplementaries:urn", ["B B", "BAB", "BBB"], {
	...
	event
		.shapeless("supplementaries:ash_brick", ["tfc:powder/wood_ash", "tfc:daub"])
		.id("setsu:supplementaries/ash_brick");

	event
		.shaped("supplementaries:antique_ink", ["T", "B", "J"], {
	...
	event
		.shaped("supplementaries:quiver", ["AA", "LR"], {
			A: "#minecraft:arrows",
			L: "minecraft:leather",
	...
	event
		.shaped("supplementaries:lunch_basket", [" C ", "CWC", "CCC"], {
