...
function setRecipesImmersiveEngineering(event) {
	...
	const addMetalPress = (output, count, ingredient, mold) => {
		event
			...
			.id(
				`setsu:immersiveengineering/metalpress/${ingredient.replace(":", ".").replace("/", ".")}/to/${output.replace(":", ".").replace("/", ".")}`
			);
	};
	const addArcFurnace = (
		output,
	...
	const addCrusher = (output, ingredient) => {
		event
			...
			.id(
				`setsu:immersiveengineering/crusher/${ingredient.replace(":", ".").replace("/", ".")}/to/${output.replace(":", ".").replace("/", ".")}`
			);
	};
	...
	for (const sheetMetal of sheetAndDoubleIngotMetals) {
		...
		event.remove({
			id: `tfc_metal_items:seq_pressing/double_ingots/${sheetMetal}`,
		});
		event.remove({ id: `tfc_metal_items:seq_pressing/sheets/${sheetMetal}` });
		event.remove({
			id: `tfc_metal_items:seq_pressing/double_sheets/${sheetMetal}`,
		});
		addArcFurnace(
			`tfc:metal/double_ingot/${sheetMetal}`,
			1,
			`tfc:metal/ingot/${sheetMetal}`,
			1,
			`tfc:metal/ingot/${sheetMetal}`,
			800
		);
		addArcFurnace(
			`tfc:metal/double_sheet/${sheetMetal}`,
			1,
			`tfc:metal/sheet/${sheetMetal}`,
			1,
			`tfc:metal/sheet/${sheetMetal}`,
			800
		);
	...
	event.remove({
		id: `tfc_metal_items:seq_pressing/double_ingots/stainless_steel`,
	});
	event.remove({ id: `tfc_metal_items:seq_pressing/sheets/stainless_steel` });
	event.remove({
		id: `tfc_metal_items:seq_pressing/double_sheets/stainless_steel`,
	});
	addArcFurnace(
	...
	event
		.shaped("immersiveengineering:alloybrick", ["SI ", "IS ", "   "], {
			S: "#forge:sandstone",
	...
	event
		.shaped("immersiveengineering:hemp_fabric", ["CYC", "SYS", "YCY"], {
			S: "#forge:rods/wooden",
	...
	event
		.shaped("immersiveengineering:cokebrick", ["CBC", "BSB", "CBC"], {
			S: "#forge:sandstone",
	...
	event
		.shaped("immersiveengineering:blastbrick", ["CBC", "BMB", "CSC"], {
	...
	event
		.shaped("immersiveengineering:conveyor_basic", ["AAA", "RLR", "AAA"], {
	...
	event
		.shaped("immersiveengineering:crate", ["ABA", "B B", "ABA"], {
			A: "thermal:tar",
	...
	event
		.shaped("minecraft:mycelium", ["CM", "SL"], {
			C: "tfc:compost",
			M: "#forge:mushrooms",
			S: "#minecraft:dirt",
	...
	event
		.shaped("3x immersiveengineering:strip_curtain", ["A", "T"], {
	...
	const addSawmill = (output, count, ingredient, secondary) => {
		event
			...
			.id(
				`setsu:immersiveengineering/sawmill/${ingredient.replace(":", ".").replace("/", ".")}/to/${output.replace(":", ".").replace("/", ".")}`
			);
	};
