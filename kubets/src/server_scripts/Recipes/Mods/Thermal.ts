...
function setRecipesThermal(event) {
	function addSawMill(output, count, ingredient) {
		event.custom({
			...
			energy: 1000,
			experience: 0.15
		}).id(`setsu:thermal/sawmill/${ingredient.replace(":", ".").replace("/", ".")}/to/${output.replace(":", ".").replace("/", ".")}`);
	}
	function addPress(output, count, ingredient, ingredientCount) {
		...
		}).id(`setsu:thermal/press/${ingredient.replace(":", ".").replace("/", ".")}/to/${output.replace(":", ".").replace("/", ".")}`);
	}
	function addCrucible(output, count, ingredient) {
		event.custom({
			...
			energy: 100000
		}).id(`setsu:thermal/crucible/${ingredient.replace(":", ".").replace("/", ".")}/to/${output.replace(":", ".").replace("/", ".")}`);
	}
	function addInsolator(wood, ingredient, chance) {
		event.custom({
			...
			energy_mod: 3.0,
			water_mod: 3.0
		}).id(`setsu:thermal/insolator/${wood}/from/${ingredient.replace(":", ".")}`);
	}
	...
	ie_Metals.forEach((ingot) => {
		addPress(`immersiveengineering:plate_${ingot}`, 1, `forge:ingots/${ingot}`, 1);
	})
	...
	event.shaped("3x thermal:energy_duct", ["LOL", "BRB", "LOL"], {
	...
	event.shaped("3x thermal:fluid_duct", ["LBL", "B B", "LBL"], {
		B: "#forge:sheets/bronze",
		L: "#forge:plates/lead"
	}).id("setsu:thermal/fluid_duct")
	event.remove({ id: "thermal:fluid_duct_windowed_4" });
	event.shaped("3x thermal:fluid_duct_windowed", ["LBL", "BWB", "LBL"], {
		B: "#forge:sheets/bronze",
		W: "#forge:glass_panes",
		L: "#forge:plates/lead"
	...
	event.shapeless("9x thermal:tar", [
		"#forge:storage_blocks/tar"
	]).id("setsu:thermal/tar_block_revert");

	event.shapeless("9x minecraft:gunpowder", [
		"thermal:gunpowder_block"
	]).id("setsu:thermal/gunpowder_block_revert");

	event.shapeless("thermal:ruby_block", [
		"9x tfc:gem/ruby"
	]).id("setsu:thermal/ruby_block");
	event.shapeless("9x tfc:gem/ruby", [
		"thermal:ruby_block"
	]).id("setsu:thermal/ruby_block_revert");

	event.shapeless("thermal:sapphire_block", [
		"9x tfc:gem/sapphire"
	]).id("setsu:thermal/sapphire_block");
	event.shapeless("9x tfc:gem/sapphire", [
		"thermal:sapphire_block"
	]).id("setsu:thermal/sapphire_block_revert");

	event.replaceOutput({ mod: "thermal" }, "minecraft:leather", "tfc:large_raw_hide");
