...
export function setRecipesTerraFirmaCraft(event: $RecipesEventJS) {
	...
	event
		.shaped("firmalife:drying_mat", ["LLL", "LLL", "LLL"], {
			L: "#minecraft:leaves",
		})
		.id("setsu:tfc/drying_mat_alt");

	event
		.shaped("tfc:rotten_compost", ["RRR", "RCR", "RRR"], {
			C: "tfc:soot",
	...
	event
		.shaped("minecraft:cobbled_deepslate", ["YYY", "YYY", "YYY"], {
			Y: "#forge:cobblestone",
		})
	...
	event
		.shaped("minecraft:furnace", ["YYY", "YCY", "LSL"], {
			Y: "#forge:cobblestone",
			S: "#tfc:rock/smooth",
			L: "tfc:fire_clay",
	...
	event
		.shaped("minecraft:blast_furnace", ["DID", "DFD", "SSS"], {
			F: "minecraft:furnace",
	...
	event
		.shaped("minecraft:lodestone", ["BBB", "BAB", "BBB"], {
	...
	event
		.shapeless("tfc:empty_jar", "minecraft:glass_bottle")
		.id("setsu:tfc/jar_from_bottle");
	event
		.shapeless("minecraft:glass_bottle", "tfc:empty_jar")
		.id("setsu:tfc/bottle_from_jar");

	event
		.shapeless("minecraft:honey_bottle", [
	...
	ItemCol.tfcDirtTypes.forEach((dirtType) => {
		tfcBucketRecipes(event, `tfc:mud/${dirtType}`, 1, "minecraft:water", 100, {
	...
	addDamageShapeless(event, "minecraft:bundle", [
		"#forge:string",
	 ...
	 * Moss
	 */
	addDamageShapeless(event, "tfc:plant/moss", [
		"#tfc:mossy_loose",
		"#tfc:axes",
	]);
	...
	event
		.shaped("tfc:rustic_windmill_blade", [" B ", "WWB", " W "], {
			B: "tfc:burlap_cloth",
			W: "tfc:stick_bunch",
		})
	...
	event
		.shaped("tfc:lattice_windmill_blade", ["GSS", "SSS", "WWW"], {
			G: "tfc:glue",
			S: "tfc:stick_bundle",
			W: "#tfc:lumber",
		})
	...
	ItemCol.tfcWoodTypesWithNether.forEach((woodType) => {
		if (woodType == "crimson" || woodType == "warped") {
			...
			event
				.shaped(`beneath:wood/water_wheel/${woodType}`, ["LWL", "WSW", "LWL"], {
			...
			event
				.shaped(`tfc:wood/water_wheel/${woodType}`, ["LWL", "WSW", "LWL"], {
					L: "#forge:treated_lumber",
	...
	event
		.shaped("tfc:hand_wheel", ["SSS", "SAS", "SSS"], {
			S: "minecraft:stick",
			A: "#tfc:axles",
		})
		.id("setsu:tfc/hand_wheel");
	event
		.shaped("tfc:hand_wheel_base", ["SSS", "SAS", "SSS"], {
			S: "#tfc:rock_knapping",
			A: "#tfc:axles",
		})
