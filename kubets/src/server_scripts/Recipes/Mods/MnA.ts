...
export function setRecipesMna(event: $RecipesEventJS) {
	 ...
	 */
	event.replaceInput(
		{ id: "mna:improvised_manaweaver_wand" },
		"#mna:improvised_manaweave_wand_caps",
		"mna:vinteum_dust"
	);
	event.replaceInput(
		{ id: "mna:improvised_manaweaver_wand" },
	...
	event
		.shaped(
			"mna:decoration/chimerite_arcane_stone_chiseled",
			[" A ", "ADA", " A "],
			{
	...
	event
		.shapeless("mna:vinteum_coated_iron", [
	...
	event
		.shapeless("mna:purified_vinteum_coated_iron", [
			"#setsu:ingot/all_iron",
	...
	event.replaceInput(
		{ id: "mna:ritual_focus_minor" },
		"minecraft:iron_nugget",
		"immersiveengineering:nugget_silver"
	);

	event.recipes.tfc
		.quern("1x mna:vinteum_dust", "mna:vinteum_ore")
		.id(`setsu:mna/tfc_quern/vinteum_dust`);
	event
		.custom({
			type: "create:milling",
			ingredients: [
	...
	event.replaceInput({ id: "mna:runeforge" }, "mna:transmuted_silver", [
		"mna:transmuted_silver",
		"mna:vinteum_ingot",
	]);

	event.recipes.tfc
		.heating("mna:vinteum_ingot", 1550)
		.resultItem("mna:superheated_vinteum_ingot");
	event.recipes.tfc
		.heating("mna:purified_vinteum_ingot", 1550)
		.resultItem("mna:superheated_purified_vinteum_ingot");

	event.remove({ id: "mna:runeforging/runesmith_hammer" });
	event.remove({ id: "mna:runescribing/runesmith_chisel" });
	event.recipes.tfc
		.anvil("mna:runesmith_hammer", "mna:vinteum_ingot", [
			"hit_last",
			"hit_not_last",
			"draw_not_last",
		])
		.tier(1)
		.bonus(true)
		.id(`setsu:mna/runesmith_hammer`);
	event.recipes.tfc
		.anvil("mna:runesmith_chisel", "mna:vinteum_ingot", [
			"punch_last",
			"shrink_not_last",
		])
		.tier(1)
		.bonus(true)
		.id(`setsu:mna/runesmith_chisel`);
	...
	const mnaTransmutation = (
		...
		event
			...
			.id(
				`setsu:mna/transmutation/${target.replace(":", "_").replace("/", "_")}/into/${output.replace(":", "_").replace("/", "_")}`
			);
	};
