declare module "packages/dev/latvian/mods/kubejs/script/data/$ExportablePackResources" {
	...
	export interface $ExportablePackResources extends $PackResources {
		...
		"getMetadataSection"<T>(
			metadataSectionSerializer0: $MetadataSectionSerializer$$Type<T>
		): T;
		"getNamespaces"(packType0: $PackType$$Type): $Set<string>;
		"getResource"(
			packType0: $PackType$$Type,
			resourceLocation1: $ResourceLocation$$Type
		...
		"listResources"(
			...
			string1: string,
			string2: string,
			resourceOutput3: $PackResources$ResourceOutput$$Type
		): void;
		"packId"(): string;
	...
	export abstract class $ExportablePackResources$$Static
...
declare module "packages/dev/latvian/mods/kubejs/script/$ScriptType" {
	...
	export class $ScriptType
		...
		implements $ScriptTypePredicate, $ScriptTypeHolder
	{
		...
		public and(
			predicate0: $Predicate$$Type<$ScriptType$$Type>
...
declare module "packages/dev/latvian/mods/kubejs/script/$ScriptSource" {
	...
	import {
		$ScriptFileInfo,
		$ScriptFileInfo$$Type,
	} from "packages/dev/latvian/mods/kubejs/script/$ScriptFileInfo";
	...
	/** Class-specific type exported by ProbeJS, use global `{Type}_` types for convenience unless there's a naming conflict. */
	export type $ScriptSource$$Type =
		| $ScriptSource
		| ((arg0: $ScriptFileInfo) => $List$$Type<string>);
	/** Global type exported for convenience, use class-specific types if there's a naming conflict. */
	declare global {
...
declare module "packages/dev/latvian/mods/kubejs/script/$ScriptFile" {
	...
	import {
		$ScriptPack,
		$ScriptPack$$Type,
	} from "packages/dev/latvian/mods/kubejs/script/$ScriptPack";
	import {
		$ScriptFileInfo,
		$ScriptFileInfo$$Type,
	} from "packages/dev/latvian/mods/kubejs/script/$ScriptFileInfo";
	import {
		$ScriptSource,
		$ScriptSource$$Type,
	} from "packages/dev/latvian/mods/kubejs/script/$ScriptSource";
	...
	export class $ScriptFile implements $Comparable<$ScriptFile> {
		...
		constructor(
			p: $ScriptPack$$Type,
			i: $ScriptFileInfo$$Type,
			s: $ScriptSource$$Type
		);

		public compareTo(o: $ScriptFile$$Type): integer;
		public load(): void;
		public redirect$dol000$preEvalProcess(
...
declare module "packages/dev/latvian/mods/kubejs/script/$ScriptTypePredicate" {
	...
	import {
		$ScriptType,
		$ScriptType$$Type,
	} from "packages/dev/latvian/mods/kubejs/script/$ScriptType";
	...
	export abstract class $ScriptTypePredicate$$Static
		implements $ScriptTypePredicate
	{
	...
	/** Class-specific type exported by ProbeJS, use global `{Type}_` types for convenience unless there's a naming conflict. */
	export type $ScriptTypePredicate$$Type =
		| $ScriptTypePredicate
...
declare module "packages/dev/latvian/mods/kubejs/script/data/$GeneratedData" {
	...
	import {
		$Lazy,
		$Lazy$$Type,
	} from "packages/dev/latvian/mods/kubejs/util/$Lazy";
...
declare module "packages/dev/latvian/mods/kubejs/script/$ScriptTypeHolder" {
	...
	export abstract class $ScriptTypeHolder$$Static
		implements $ScriptTypeHolder {}
	/** Class-specific type exported by ProbeJS, use global `{Type}_` types for convenience unless there's a naming conflict. */
	export type $ScriptTypeHolder$$Type =
		| $ScriptTypeHolder
		| (() => $ScriptType$$Type);
	/** Global type exported for convenience, use class-specific types if there's a naming conflict. */
	declare global {
...
declare module "packages/dev/latvian/mods/kubejs/script/$ScriptManager" {
	import {
		$Context,
		$Context$$Type,
	} from "packages/dev/latvian/mods/rhino/$Context";
	...
	export class $ScriptManager implements $ClassShutter {
		...
		public handler$doj000$pjs$captureClass(
			name: string,
			error: boolean,
...
declare module "packages/dev/latvian/mods/kubejs/script/$ScriptFileInfo" {
	...
	import {
		$ScriptPackInfo,
		$ScriptPackInfo$$Type,
	} from "packages/dev/latvian/mods/kubejs/script/$ScriptPackInfo";
...
declare module "packages/dev/latvian/mods/kubejs/script/data/$DataPackEventJS" {
	...
	export class $DataPackEventJS extends $EventJS {
		constructor(
			d: $VirtualKubeJSDataPack$$Type,
			rm: $MultiPackResourceManager$$Type
		);
...
declare module "packages/dev/latvian/mods/kubejs/script/$ConsoleLine$SourceLine" {
	...
	export class $ConsoleLine$SourceLine extends $Record {
		...
		public static write(
			...
			sourceLine: $ConsoleLine$SourceLine$$Type
		): void;
	}
	/** Class-specific type exported by ProbeJS, use global `{Type}_` types for convenience unless there's a naming conflict. */
...
declare module "packages/dev/latvian/mods/kubejs/script/data/$VirtualKubeJSDataPack" {
	 ...
	 * Loading the class using require() will not throw an error, but the class will be undefined.
	 */
	export class $VirtualKubeJSDataPack
		...
		implements $ExportablePackResources
	{
		...
		public getMetadataSection<T>(
			serializer: $MetadataSectionSerializer$$Type<T>
		): T;
...
declare module "packages/dev/latvian/mods/kubejs/script/$ConsoleLine" {
	...
	import {
		$LogType,
		$LogType$$Type,
	} from "packages/dev/latvian/mods/kubejs/util/$LogType";
	import {
		$Collection,
		$Collection$$Type,
	} from "packages/java/util/$Collection";
	import {
		$ConsoleLine$SourceLine,
		$ConsoleLine$SourceLine$$Type,
	} from "packages/dev/latvian/mods/kubejs/script/$ConsoleLine$SourceLine";
	import { $List, $List$$Type } from "packages/java/util/$List";
	import {
		$ConsoleJS,
		$ConsoleJS$$Type,
	} from "packages/dev/latvian/mods/kubejs/util/$ConsoleJS";
	...
	export class $ConsoleLine {
		...
		public static writeToNet(
			buf: $FriendlyByteBuf$$Type,
			line: $ConsoleLine$$Type
		): void;
...
declare module "packages/dev/latvian/mods/kubejs/script/$ScriptPack" {
	...
	import {
		$ScriptManager,
		$ScriptManager$$Type,
	} from "packages/dev/latvian/mods/kubejs/script/$ScriptManager";
	import {
		$ScriptPackInfo,
		$ScriptPackInfo$$Type,
	} from "packages/dev/latvian/mods/kubejs/script/$ScriptPackInfo";
