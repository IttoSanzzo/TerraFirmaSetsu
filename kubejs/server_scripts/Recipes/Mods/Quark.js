...
function setRecipesQuark(event) {
	// 0. Replaces
	...
	event.remove({ id: "quark:building/crafting/compressed/stick_block_uncompress" });
	event.replaceInput({ id: "quark:building/crafting/paper_lantern" }, "minecraft:bamboo", "#minecraft:wooden_buttons")
	event.replaceInput({ id: "quark:tools/crafting/abacus" }, "tfc:metal/ingot/wrought_iron", "#forge:bones")
	...
	event.shaped("quark:deepslate_furnace", ["YYY", "YCY", "LSL"], {
	...
	*/
	...
	const reVertical = (mod, names) => {
		names.forEach((name) => {
			event.remove({ id: `quark:building/crafting/vertplanks/vertical_${name}_planks` });
			event.remove({ id: `quark:building/crafting/vertplanks/vertical_${name}_planks_revert` });
			event.shaped(`3x quark:vertical_${name}_planks`, ["W", "W", "W"], {
				W: `${mod}:${name}_planks`
			});
			event.shapeless(`${mod}:${name}_planks`, [`quark:vertical_${name}_planks`]);
		});
	};
	...
	reWoods.forEach((wood) => {
		...
		addDamageShapeless(event, `3x quark:${wood}_planks`, [`#quark:${wood}_logs`, "#tfc:metal/saws"]);
	});
}
