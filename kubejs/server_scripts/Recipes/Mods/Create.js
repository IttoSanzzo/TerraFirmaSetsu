...
function setRecipesCreate(event) {
	function createAddCutting(output, count, ingredient) {
		...
		}).id(`setsu:create/cutting/${ingredient.replace(":", ".").replace("/", ".")}/to/${output.replace(":", ".").replace("/", ".")}`)
	}
	function createAddPressing(output, ingredient) {
		...
		}).id(`setsu:create/pressing/${ingredient.replace(":", ".").replace("/", ".")}/to/${output.replace(":", ".").replace("/", ".")}`);
	}
	function createAddRolling(output, count, ingredient) {
		...
		}).id(`setsu:create/rolling/${ingredient.replace(":", ".").replace("/", ".")}/to/${output.replace(":", ".").replace("/", ".")}`);
	}
	...
	ItemCol.tfcWoodTypes.forEach((log) => {
		...
		createAddCutting(`tfc:wood/lumber/${log}`, 8, `tfc:wood/stripped_log/${log}`);
	...
	for (const rodMetal of rodMetals) {
		...
		createAddRolling(`tfc:metal/rod/${rodMetal}`, 1, `tfc:metal/ingot/${rodMetal}`);
	}
	createAddRolling(`firmalife:metal/rod/chromium`, 1, `firmalife:metal/ingot/chromium`);
	createAddRolling(`firmalife:metal/rod/stainless_steel`, 1, `firmalife:metal/ingot/stainless_steel`);
	...
	ItemCol.allDyes.forEach((color) => {
		if (color == "cyan" || color == "gray" || color == "black")
			return;
		event.custom({
			type: "create:milling",
			results: [
	...
	event.shaped("create:belt_connector", ["AAA", "RLR", "AAA"], {
	...
	event.replaceInput({ id: "create:crafting/schematics/schematicannon" }, "minecraft:iron_block", "tfc:metal/double_ingot/wrought_iron");
	event.replaceInput({ id: "create:crafting/kinetics/large_water_wheel" }, "#minecraft:planks", "#forge:treated_lumber");
	event.replaceInput({ id: "create:crafting/appliances/clipboard" }, "#minecraft:planks", "#tfc:lumber");
	...
	event.shaped("6x create:belt_connector", ["AAA", "AAA"], {
	...
	event.replaceInput({ id: "create:haunting/soul_lantern" }, "minecraft:lantern", "#tfc:lamps");
